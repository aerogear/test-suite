...
module.exports = async (appName, serviceName, params) => {
    ...
    switch (serviceName) {
        case syncServiceName:
            ...
            await page.type('#root_syncServerConfig_url', `https://${params.syncUrl}`)
            break
        case idmServiceName:
            expect(params).to.include.all.keys('idmRealmName', 'idmUser', 'idmPassword')
        ...
        case mssServiceName:
            ...
            break
        case upsServiceName:
            expect(params).to.include.all.keys('upsAndroidServerKey', 'upsAndroidSenderId')
            await page.waitForSelector('#root_CLIENT_TYPE')
            await page.select('#root_CLIENT_TYPE', 'Android')
            await page.type('#root_platformConfig_googlekey', params.upsAndroidServerKey)
