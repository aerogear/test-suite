const { login, createApp, deleteApp } = require("../utils/mdc");
...
describe("Testing MDC Apps filtering", () => {
  ...
  afterEach(async () => {
    await page.$eval(
      ".toolbar-pf-filter input.toolbarFilter",
      el => (el.value = "")
    );
  });
  ...
  it("MDC overview page should display only test* apps", async () => {
    ...
    await page.waitForFunction(
      filter1 =>
        [...document.querySelectorAll(".card-pf-title > h1")].every(el =>
          el.innerText.includes(filter1)
        ),
      {},
      filter1
    );
