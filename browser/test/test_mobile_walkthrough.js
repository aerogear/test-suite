...
describe('Test Mobile Walkthrough in Solution Explorer', () => {
    ...
    it('should redirect to Solution Explorer', async () => {
        ...
        await page.waitForFunction(
            () => [...document.querySelectorAll('.card a')].find(header => header.textContent.includes('Solution Explorer'))
        )
        const solutionExplorerUrl = await page.$$eval(
            '.card a',
            cards => cards.map(card => card.href).find(href => href.includes('tutorial-web-app'))
        )
        ...
        const newTarget = await browser.waitForTarget(target => target.url() === solutionExplorerUrl);
        page = await newTarget.page();
        await page.waitForSelector('#mobile-walkthrough-walkthroughs-1-ionic-showcase article')
    })
