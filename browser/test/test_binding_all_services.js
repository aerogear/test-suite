...
describe("Test binding of all services", () => {
  ...
  it("binding Identity Management service should complete successfully", async () => {
    await bindServiceToApp(appName, idmServiceName, {
      idmRealmName,
      idmUser,
      idmPassword
    });
  ...
  it("binding Unified Push Server should complete successfully", async () => {
    await bindServiceToApp(appName, upsServiceName, {
      upsAndroidSenderId,
      upsAndroidServerKey,
      upsIosCertificate,
      upsIosPassword
  ...
  it("mobile-services.json configuration should be updated with configuration from all services", async () => {
    ...
    await page.waitForFunction(
      () =>
        JSON.parse(
          document.querySelector(".mobile-client-config pre").textContent
        ).services.length === 4
    );
    ...
    for (service of services) {
      switch (service.name) {
        case "keycloak":
          ...
          expect(service.config).to.have.keys(
            "auth-server-url",
            "confidential-port",
            "public-client",
            "realm",
        ...
        case "push":
          ...
          expect(service.config.android).to.include.all.keys(
            "variantId",
            "variantSecret"
          );
          break;
        ...
        case "sync-app":
          ...
          expect(service).to.have.deep.property("config", {
