...
import { BackupHelper } from "../util/backup-helper";
...
describe("Backup & Restore", function () {
    ...
    before(async () => {
        ...
        const credentials = await kh.readSecret("s3-credentials", BACKUPS_NAMESPACE);
        // connect to the s3 bucket
        sh = S3Helper.new(decode(credentials.data.AWS_ACCESS_KEY_ID), decode(credentials.data.AWS_SECRET_ACCESS_KEY));
    ...
    it("should backup all namespaces, ups database and mss database", async () => {
        ...
        upsBackup = upsBackups.find(b => /unifiedpush-\d{2}_\d{2}_\d{2}.pg_dump.gz$/.test(b.file));
        // as for the ups
        mssBackup = mssBackups.find(b => /mobile_security_service-\d{2}_\d{2}_\d{2}.pg_dump.gz$/.test(b.file));
    ...
    // it("", () => {
    //   resourceBackup = {
    //     bucket: "rhmi-qe1-backups",
    //     file: "backups/openshift/resources/2019/11/13/resources_10_59_52.tar.gz"
    //   };
    //   upsBackup = {
    //     bucket: "rhmi-qe1-backups",
    //     file:
    //       "backups/unifiedpush/postgres/2019/11/13/openshift-mobile-unifiedpush.unifiedpush-10_59_53.pg_dump.gz"
    //   };
    //   mssBackup = {
    //     bucket: "rhmi-qe1-backups",
    //     file:
    //       "backups/mss/postgres/2019/11/13/openshift-mobile-security-service.mobile_security_service-10_59_52.pg_dump.gz"
    //   };
    //   mdcNamespace = `openshift-${MDC_NAMESPACE}`;
    //   mssNamespace = `openshift-${MSS_NAMESPACE}`;
    //   upsNamespace = `openshift-${UPS_NAMESPACE}`;
    // });
    ...
    it("should restore mss namespace", async () => {
        ...
        await restoreNamespace(kh, tmpdir, mssNamespace, ["services", "routes", "deployments"], ["addressspaceschemas"]);
    ...
    it("should restore mss database", async () => {
        ...
        await kh.waitForDeploymentToReconcile("mobile-security-service-db", mssNamespace);
        // scale the operator down
        await kh.scaleDeploymentAndWait("mobile-security-service-operator", mssNamespace, 0);
        // scale mss down
        await kh.scaleDeploymentAndWait("mobile-security-service", mssNamespace, 0);
        // restore mss database
        const databasePod = (await kh.listPod(mssNamespace, "name=mobilesecurityservicedb"))[0];
        // find the mss dump
        const re = /\.mobile_security_service-\d{2}_\d{2}_\d{2}\.pg_dump$/;
        const file = fs.readdirSync(tmpdir).find(f => re.test(f));
        // restore the dump
        await kh.exec(mssNamespace, databasePod.metadata.name, databasePod.spec.containers[0].name, ["bash", "-lc", "psql mobile_security_service"], fs.createReadStream(path.join(tmpdir, file)));
        log.info("mss db restored");
        // scale mss up
        await kh.scaleDeploymentAndWait("mobile-security-service", mssNamespace, 1);
        // scale the operator up
        await kh.scaleDeploymentAndWait("mobile-security-service-operator", mssNamespace, 1);
    });
    it("should restore ups namespace", async () => {
        // restore the namespace
        await restoreNamespace(kh, tmpdir, upsNamespace, ["services", "routes", "deploymentconfigs"], ["addressspaceschemas"]);
    ...
    it("should restore ups database", async () => {
        ...
        await kh.waitForDeploymentConfigToReconcile("unifiedpush-postgresql", upsNamespace);
        // scale the operator down
        ...
        // restore mss database
        const databasePod = (await kh.listPod(upsNamespace, "deploymentconfig=unifiedpush-postgresql"))[0];
        // find the mss dump
        const re = /\.unifiedpush-\d{2}_\d{2}_\d{2}\.pg_dump$/;
        const file = fs.readdirSync(tmpdir).find(f => re.test(f));
        // restore the dump
        await kh.exec(upsNamespace, databasePod.metadata.name, databasePod.spec.containers[0].name, ["bash", "-lc", "psql unifiedpush"], fs.createReadStream(path.join(tmpdir, file)));
    ...
    it("should restore mdc namespace", async () => {
        await restoreNamespace(kh, tmpdir, mdcNamespace, ["services", "routes", "deploymentconfigs"], [
            "addressspaceschemas",
        ...
        const script = path.join(__dirname, "../fixtures/mobile-developer-console-restoration.sh");
